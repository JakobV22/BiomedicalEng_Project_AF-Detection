...
function updateProgress()
    ...
    if isempty(count)
        count = 0;
        total = evalin('base', 'numCombos'); % Gesamtzahl aus Workspace holen
    end
    count = count + 1;
    percent = (count / total) * 100;
    fprintf('\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b'); % 16 Backspaces löschen
...
parfor i = 1:numCombos
    ...
    Xmat = table2array(X);  % Tabelle in Matrix umwandeln
    Y = categorical(trainingTable.(targetVar)); % Zielvariable kategorisch

    try
        Mdl = fitcensemble(Xmat, Y, ...
            'Method', 'RUSBoost', ...
